  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-slate-200 dark:border-zinc-800 pb-8">
        <div>
          <h1 className="text-2xl font-black text-[#112a46] dark:text-white uppercase tracking-tighter">Akademisyen Envanteri</h1>
          <p className="text-slate-500 dark:text-zinc-400 mt-1 text-sm font-medium">Sistemde kayıtlı olan tüm öğretim üyelerinin yönetim paneli.</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
            <select className="px-5 py-3.5 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-400 rounded text-xs font-bold uppercase tracking-wider focus:outline-none focus:ring-1 focus:ring-[#112a46]">
                ...
                <tr key={instructor.id} className="hover:bg-slate-50/30 dark:hover:bg-zinc-800/20 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <p className="text-[#112a46] dark:text-zinc-200 font-bold text-sm leading-tight uppercase tracking-tight">{instructor.name}</p>
                        <p className="text-slate-400 dark:text-zinc-500 text-[10px] font-black mt-1 tracking-widest uppercase">{instructor.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-slate-400 dark:text-zinc-500 text-[10px] font-black uppercase tracking-widest">{instructor.university}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} size={12} className={`${s <= Math.round(instructor.rating) ? 'text-slate-700 dark:text-zinc-300 fill-current' : 'text-slate-200 dark:text-zinc-800'}`} />
                        ))}
                        <span className="ml-2 text-[#112a46] dark:text-white font-black text-xs">{instructor.rating}</span>
                      </div>
                      <span className="text-slate-400 dark:text-zinc-500 text-[9px] font-black uppercase tracking-[0.15em]">{instructor.reviewCount} Kayıtlı Yorum</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
