  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-slate-200 dark:border-zinc-800 pb-8">
        <div>
          <h1 className="text-2xl font-black text-kk-blue dark:text-white uppercase tracking-tighter">Müfredat Veritabanı</h1>
          <p className="text-kk-text-muted dark:text-zinc-400 mt-1 text-sm font-medium">Sistemdeki tüm derslerin, AKTS kredilerinin ve içerik bilgilerinin merkezi yönetimi.</p>
        </div>
                ...
                <tr key={course.id} className="hover:bg-slate-50/30 dark:hover:bg-zinc-800/20 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <p className="text-kk-blue dark:text-zinc-200 font-bold text-sm leading-tight uppercase tracking-tight">{course.code}</p>
                        <p className="text-kk-text-muted dark:text-zinc-500 text-[10px] font-black mt-1 tracking-widest uppercase">{course.name}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3">
                         <div className="h-1.5 w-24 bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                            <div className="h-full bg-kk-blue-light dark:bg-slate-300" style={{ width: `${(course.rating/5)*100}%` }}></div>
                         </div>
                         <span className="text-kk-blue dark:text-white font-black text-xs">{course.rating}</span>
                      </div>
                      <span className="text-kk-text-muted dark:text-zinc-500 text-[9px] font-black uppercase tracking-[0.15em]">{course.reviewCount} Değerlendirme Kaydı</span>
                    </div>
                  </td>
