...
import { User, ShieldCheck, ChevronRight, ArrowLeft, Star } from "lucide-react";
        ...
        <div className="max-w-6xl w-full mx-auto px-4 py-12 flex-1">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside className="w-full lg:w-64 flex flex-col gap-1">
              <SettingsLink href="/settings"          icon={<User size={18} />}       label="Profil Bilgileri"  active={pathname === "/settings"} />
              <SettingsLink href="/settings/security"          icon={<ShieldCheck size={18} />} label="Güvenlik ve Şifre"    active={pathname === "/settings/security"} />
              <SettingsLink href="/settings/degerlendirmelerim" icon={<Star size={18} />}       label="Değerlendirmelerim" active={pathname === "/settings/degerlendirmelerim"} />

              <div className="mt-6 p-5 bg-white/80 backdrop-blur-md border border-white/60 rounded-lg shadow-sm">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-9 h-9 rounded bg-kk-blue flex items-center justify-center text-kk-gold text-xs font-bold">
                    {initials}
                  </div>
                  <div>
                    <p className="text-xs font-bold text-kk-blue tracking-tight">{user?.full_name ?? "—"}</p>
                    <p className="text-[10px] text-kk-text-muted font-medium uppercase tracking-widest">
                      {user ? (ROLE_LABEL[user.role] ?? user.role) : "—"}
                    </p>
                  </div>
                </div>
                <div className="border-t border-kk-blue/10 pt-4">
                  <button
                    onClick={handleLogout}
                    className="w-full py-2 text-[11px] font-bold text-red-600 border border-red-100 hover:bg-red-50 rounded transition-colors uppercase tracking-wider cursor-pointer"
                  >
                    Oturumu Kapat
                  </button>
                </div>
  ...
  return (
    <Link
      ...
      className={`flex items-center justify-between px-4 py-3 rounded transition-all duration-200 group ${
        active ? "bg-kk-blue text-kk-beige shadow-sm" : "hover:bg-white/60 text-kk-text-muted hover:text-kk-blue border border-transparent"
      }`}
    >
      <div className="flex items-center gap-3">
        <span className={active ? "text-kk-beige" : "text-kk-text-muted group-hover:text-kk-blue transition-colors"}>{icon}</span>
